...
function solve(start: Coords, endX: number, endY: number): number {
  const queue: [Coords, number][] = [[start, 0]];
  const visited = new Matrix(width, height, cycles);
  visited.set(start, 1);
  ...
  while (queue.length > 0 && !found) {
    ...
    for (const dir of dirs) {
      ...
      const newCoords: Coords = [newX, newY, newCycle];
      if (map.get(newCoords) === 0 && visited.get(newCoords) === 0) {
        visited.set(newCoords, 1);
        queue.push([newCoords, newDist]);
...
const dist2 = solve([endX, endY, dist1 % cycles], startX, startY);
const dist3 = solve([startX, startY, (dist1 + dist2) % cycles], endX, endY);
console.log(dist1, dist2, dist3, dist1 + dist2 + dist3);
