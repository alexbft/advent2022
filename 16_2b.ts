...
let maxVolume = 0;
function updateDynValue(map: DynMap, valve1Code: string, valve2Code: string, valveMask: number, volume: number, time: number) {
  if (valve2Code < valve1Code) {
    const tmp = valve1Code;
    valve1Code = valve2Code;
    valve2Code = tmp;
  }
  const mapCode = `${valve1Code}${valve2Code}`;
  if (volume + upperLimit[valveMask] * (time - 2) <= maxVolume) {
    return;
  }
...
for (let vMask = 0; vMask < (1 << nonZeroValves.length); ++vMask) {
  let total = 0;
  for (let v = 0; v < nonZeroValves.length; ++v) {
    if ((vMask & (1 << v)) === 0) {
      total += flow.get(nonZeroValves[v])!;
    }
  }
  upperLimit.push(total);
}

let dyn: DynMap = new Map();
updateDynValue(dyn, 'AA', 'AA', 0, 0, 1000);
for (let time = 26; time > 1; --time) {
    ...
    for (const valveMask of maskMap.keys()) {
      ...
      for (const nextValve1Code of paths1) {
        for (const nextValve2Code of paths2) {
          ...
          updateDynValue(newDyn, nextValve1Code, nextValve2Code, nextValveMask, nextVolume, time);
        }
      }
    }
  }
...
console.log(`Elapsed: ${Date.now() - sTime}`);
